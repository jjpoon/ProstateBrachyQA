...
try
    ...
    if numel(handles.images) >= testNum
        if ~isempty(handles.images{testNum})
            ...
            if ~isempty(handles.upperScaleReading{testNum}) && ~isempty(handles.lowerScaleReading{testNum})
                % Scale readings were inputted
                [result,baselineVals,newVals,freq] = lateralResolutionTestAuto(handles.images{testNum}{:},...
                    'UpperScale',handles.upperScaleReading{testNum},'LowerScale',handles.lowerScaleReading{testNum},...
                    'AxesHandle',axesHandle);
            else
                % Read scale automatically from image
                [result,baselineVals,newVals,freq] = lateralResolutionTestAuto(handles.images{testNum}{:},'AxesHandle',axesHandle);
            end
...
try
    ...
    if numel(handles.images) >= testNum
        if ~isempty(handles.images{testNum})
            ...
            if ~isempty(handles.upperScaleReading{testNum}) && ~isempty(handles.lowerScaleReading{testNum})
                % Scale readings were inputted
                [result,knownVal,measuredVals,freq] = lateralDistanceTestAuto(handles.images{testNum}{:},...
                    'UpperScale',handles.upperScaleReading{testNum},'LowerScale',handles.lowerScaleReading{testNum},...
                    'AxesHandle',axesHandle);
            else
                % Read scale automatically from image
                [result,knownVal,measuredVals,freq] = lateralDistanceTestAuto(handles.images{testNum}{:},'AxesHandle',axesHandle);
            end
...
try
    ...
    if numel(handles.images) >= testNum
        if ~isempty(handles.images{testNum})
            ...
            if ~isempty(handles.upperScaleReading{testNum}) && ~isempty(handles.lowerScaleReading{testNum})
                % Scale readings were inputted
                [result,knownVal,measuredVal,freq] = areaTestAuto(handles.images{testNum}{:},...
                    'UpperScale',handles.upperScaleReading{testNum},'LowerScale',handles.lowerScaleReading{testNum},...
                    'AxesHandle',axesHandle);
            else
                % Read scale automatically from image
                [result,knownVal,measuredVal,freq] = areaTestAuto(handles.images{testNum}{:},'AxesHandle',axesHandle);
            end
            ...
            % Show frequency
            set(handles.area_text_freq,'String',num2str(freq));
...
try
    ...
    if numel(handles.images) >= testNum
        if ~isempty(handles.images{testNum})
            ...
            if ~isempty(handles.upperScaleReading{testNum}) && ~isempty(handles.lowerScaleReading{testNum})
                % Scale readings were inputted
                [result,knownVal,measuredVal,freq] = volumeTestAuto(handles.images{testNum}{:},...
                    'UpperScale',handles.upperScaleReading{testNum},'LowerScale',handles.lowerScaleReading{testNum},...
            ...
            % Show frequency
            set(handles.volume_text_freq,'String',num2str(freq));
...
try
    ...
    if numel(handles.images) >= testNum
        if ~isempty(handles.images{testNum})
            ...
            if ~isempty(handles.upperScaleReading{testNum}) && ~isempty(handles.lowerScaleReading{testNum})
                % Scale readings were inputted
                [result,knownVal,measuredVal,freq] = volumeTestFormula(handles.images{testNum}{:},...
            ...
            % Show frequency
            set(handles.volumeFormula_text_freq,'String',num2str(freq));
...
try
    excelFile = fullfile(pwd,'Log\','ProstateBrachyQA Log.xlsx');
    % If ProstateBrachyQA Log file doesn't exist, create it
    if ~exist(excelFile,'file')
        xlswrite(excelFile,cell(1));
    end
    % Let user select export file, select ProstateBrachyQA Log.xlsx by default
    [filename,pathname] = uigetfile({'*.xlsx';'*.xls'},'Select Export File',excelFile);
    ...
    if filename ~= 0
        ...
        Workbook = Open(Workbooks,fullfile(pathname,filename));
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            if ~isempty(lastRow)
                ...
                Sheet.get('Cells',1,1).Interior.ColorIndex = 6;
                % Axial plane headers
                titleAxial = Sheet.get('Range',Sheet.get('Cells',1,2),Sheet.get('Cells',1,numel(colHeaders)+3));
                titleAxial.MergeCells = 1;
                titleAxial.Value = 'Axial Plane';
                freqHeaderAxial = Sheet.get('Cells',2,2);
                freqHeaderAxial.Value = 'Freq (MHz)';
...
freqAxial = get(handles.lateralResolution_text_freqAxial,'String');
freqLong = get(handles.lateralResolution_text_freqLong,'String');
exportDate = datestr(clock,'yyyymmdd_HHMMSS');
exportFolder = fullfile(pwd,'Log\Images',date);
if ~exist(exportFolder,'dir')
    mkdir(exportFolder);
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            for f = 1:numel(axialFreqs)
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Lateral Resolution (Axial)'];
                % Set/update chart position
                if f == 1
                    chartCell = Sheet.get('Cells',numRows+2,1);
                else
            ...
            for g = 1:numel(longFreqs)
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Lateral Resolution (Longitudinal)'];
                % Set/update chart position
                if g == 1
                    chartCell = Sheet.get('Cells',numRows+2,chartShape{1}.BottomRightCell.Column+1);
                else
...
freqAxial = get(handles.lateralDistance_text_freqAxial,'String');
freqLong = get(handles.lateralDistance_text_freqLong,'String');
exportDate = datestr(clock,'yyyymmdd_HHMMSS');
exportFolder = fullfile(pwd,'Log\Images',date);
if ~exist(exportFolder,'dir')
    mkdir(exportFolder);
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            for f = 1:numel(axialFreqs)
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Lateral Distance (Axial)'];
                % Set/update chart position
                if f == 1
                    chartCell = Sheet.get('Cells',numRows+2,1);
                else
            ...
            for g = 1:numel(longFreqs)
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Lateral Distance (Longitudinal)'];
                % Set/update chart position
                if g == 1
                    chartCell = Sheet.get('Cells',numRows+2,chartShape{1}.BottomRightCell.Column+1);
                else
...
freqVal = get(handles.area_text_freq,'String');
exportDate = datestr(clock,'yyyymmdd_HHMMSS');
exportFolder = fullfile(pwd,'Log\Images',date);
if ~exist(exportFolder,'dir')
    mkdir(exportFolder);
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            if ~isempty(lastRow)
                ...
                Sheet.get('Cells',1,1).Interior.ColorIndex = 6;
                % Frequency header
                freqHeader = Sheet.get('Cells',1,2);
                freqHeader.Value = 'Freq (MHz)';
                % Column headers
            ...
            for f = 1:numel(freqs)
                ...
                % Clear default data
                Workbook.ActiveChart.ChartArea.ClearContents;
                % Get row numbers for this frequency
                if iscell(freqColumn.Value)
                    [~,freqRows] = find([freqColumn.Value{:}]==freq);
                else
                    [~,freqRows] = find([freqColumn.Value]==freq);
                end
                % Get chart shape
                ...
                for s = 1:numel(objects)
                    ...
                    if ~isempty(intersect(rowNums,freqRows))
                        ...
                        for n = 1:numel(rowNums)
                            % Only add data for current frequency
                            if freqColumn.Cells.Item(rowNums(n)).Value == freq
                                if isempty(dateRange)
                                    dateRange = dateColumn.Cells.Item(rowNums(n));
                                else
                        ...
                        for n = 1:numel(rowNums)
                            ...
                            if freqColumn.Cells.Item(rowNums(n)).Value == freq
                                if isempty(valRange)
                                    valRange = valColumn.Cells.Item(rowNums(n));
                                else
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Area'];
                % Set/update chart position
                if f == 1
                    chartCell = Sheet.get('Cells',numRows+2,1);
                else
...
freqVal = get(handles.volume_text_freq,'String');
exportDate = datestr(clock,'yyyymmdd_HHMMSS');
exportFolder = fullfile(pwd,'Log\Images',date);
if ~exist(exportFolder,'dir')
    mkdir(exportFolder);
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            for n = 1:numel(colHeaders)-1
                ...
                if ~isempty(fieldCol)
                    % Column exists, write the value in new row
                    Sheet.get('Cells',dateCell.Row,fieldCol).Value = val;
                end
            end
            ...
            for f = 1:numel(freqs)
                ...
                Workbook.ActiveChart.HasTitle = 1;
                Workbook.ActiveChart.ChartTitle.Text = [num2str(freq) ' MHz Volume'];
                % Set/update chart position
                if f == 1
                    chartCell = Sheet.get('Cells',numRows+2,1);
                else
...
freqVal = get(handles.volumeFormula_text_freq,'String');
exportDate = datestr(clock,'yyyymmdd_HHMMSS');
exportFolder = fullfile(pwd,'Log\Images',date);
if ~exist(exportFolder,'dir')
    mkdir(exportFolder);
    ...
    if filename ~= 0
        ...
        if Workbook.ReadOnly == 0
            ...
            for imNum = 1:numel(handles.volumeFormula_axes_list)
                ...
                handles.exportImages{handles.testNum}{imNum} = im;
                filename = fullfile(exportFolder,[exportDate '_VolumeFormula_' num2str(imNum) '.bmp']);
                imwrite(im,filename);
                [~,imageCol] = find(strcmp(xlData(1,:),'Images'),1);
                imageCell = Sheet.get('Cells',dateCell.Row,imageCol+imNum-1);
                % Add link to image
